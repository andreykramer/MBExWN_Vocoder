...
import MBExWN_NVoc.sig_proc.swipe as swipe
import numpy as np
...
def calc_f0_swipe(infile, outfile, f0min=50, f0max=450, time_step=0.005,
    ...
    if txt_format:
        ...
        data_dict['date'] = datetime.datetime.now().strftime(
            '%Y-%m-%d %H:%M.%S')
...
def find_audios_in_dir(inpath, audio_format="wav"):
    return glob.glob(inpath + f"*.{audio_format}")


def get_output_filepath(inpath, outdir, format):
    if format is None:
        format = "f0"
    in_basename = os.path.splitext(os.path.basename(inpath))[0]
    out_filepath = os.path.join(outdir, f"{in_basename}.{format}")
    return out_filepath
...
if __name__ == "__main__":
    ...
    parser.add_argument("inpath", help="input file or directory.")
    parser.add_argument("--outpath", default=None,
                        help="sdif output file name (def: %(default)s )")
    parser.add_argument("--audio_format", default="wav", type=str,
                        help="format of audio to search in [inpath] if [inpath] is a directory (def: %(default)s )")
    ...
    if os.path.isfile(args.inpath):
        ...
        infiles = find_audios_in_dir(args.inpath)
        outfiles = [get_output_filepath(
            i, outdir, args.format) for i in infiles]

    for infile, outfile in tqdm(zip(infiles, outfiles), total=len(infiles)):
        calc_f0_swipe(infile, outfile, f0min=args.f0min, f0max=args.f0max,
